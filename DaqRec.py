...
def mainRecord():
    ...
    # other stuff
    samples_per_channel = 5000 # Not important as long as smaller than 10e3*amount_of_channels
    ...
    try:
        ...
        # Finish and clean in case something is going on
        hat.a_in_scan_stop()
        ...
        read_request_size = 500#samples_per_channel
        ...
        while hat_out.dio_input_read_bit(0)==change_DI:#True:
            ...
            if samples_read_per_channel > 0:
                ...
                idx_last = samples_read_per_channel * num_channels - num_channels
                for i in range(num_channels):
                    # Write the last row to the csv
                    value = read_result.data[idx_last+i]
                    if disp_flag:
                ...
                csvfile.flush()
            #else:
                #empty_read = read_result
                #empty_read = hat.a_in_scan_read(read_request_size, timeout)
            # Wait the specified interval between reads.
                #sleep(0.1)
            #while (time.clock_gettime(t_clk)-t_last)<control_period:
            #    minimum_operation = 0
                # it could be reading the buffer here
                # and storing the data in the array
                # or wirting csv
                # but it doesn't matter because it is only the remaining time
